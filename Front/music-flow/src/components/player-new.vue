<template>
  ...
  <div class="music-player">
    ...
    <div class="error-message" v-if="!isLoading && !isContentReady">
        Ошибка загрузки трека
    </div>

    <!-- Основной контент -->
    <div class="player-container" v-show="!isLoading && isContentReady">
...
methods: {
    ...
    this.$nextTick(() => {
      ...
      if (currentTimeEl) {
        currentTimeEl.textContent = this.formatTime(this.currentTime || 0);
      }
      if (durationEl) {
        durationEl.textContent = this.formatTime(this.duration || 0);
      }
    });
  },
    ...
    // Дополнительная логика обновления UI если нужно
    console.log('Player state updated:', isPlaying ? 'Playing' : 'Paused');
  },
  ...
  async sendPlayCommand() {
    try {
      // Решение ошибки "play() failed because the user didn't interact"
          ...
          case 'init':
                    if (data.track_url) {
                      await this.loadTrack(data.track_url, async () => {
                        ...
                        this.currentAudio.currentTime = data.current_time || 0;
                        
                        // Не пытаемся автоматически играть - ждем команды от пользователя
                        this.isPlaying = false;
                        await this.updatePlayerUI(false);
                      });
                    }
  ...
  async sendPrevTrack() {
    ...
    async playEntranceAnimation() {
     ...
     async loadTrack(trackUrl, callback) {
      try {
        ...
        // Анимация скрытия старого контента (если это не первая загрузка)
        ...
        // Обновление UI
        this.currentTrackTitle = data.title || 'Неизвестный трек';
        this.currentArtist = data.artist || 'Неизвестный исполнитель';
        this.$refs.coverImage.src = data.cover || '';

        // Настройка аудио
        this.currentAudio.src = `/api/tracks${data.stream_url}`;
        
        // Ожидание готовности аудио
        ...
        await this.playContentAnimation();
    ...
    async playContentAnimation() {
      // Проверка что анимация еще не проигрывалась
      ...
      gsap.set([this.$refs.frame, this.$refs.coverImage, this.$refs.songInfo], {
        opacity: 0,
        y: 20
      });
      ...
      tl.to(this.$refs.frame, {
        opacity: 1,
        y: 0,
        duration: 0.8,
        ease: "back.out(1.7)"
      })
      .to(this.$refs.coverImage, {
        opacity: 1,
        scale: 1,
        duration: 0.8,
        ease: "power2.out"
      }, "-=0.4")
      .to(this.$refs.songInfo, {
        opacity: 1,
        y: 0,
        duration: 0.6,
        ease: "power2.out"
      }, "-=0.3")
      .to(this.$refs.timeBar?.$el, {
        scaleX: 1,
        duration: 0.8,
        ease: "power3.out"
      }, "-=0.4")
      .to(this.$refs.playControl?.$el, {
        opacity: 1,
        y: 0,
        duration: 0.7,
        ease: "elastic.out(1, 0.5)"
      ...
      if (this.$refs.volumeControl?.$el) {
        tl.to(this.$refs.volumeControl.$el, {
          opacity: 1,
          y: 0,
          duration: 0.7,
          ease: "elastic.out(1, 0.5)"
